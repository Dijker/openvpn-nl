...
init_crypto_pre (struct context *c, const unsigned int flags)
{
  ...
  if (c->options.use_prediction_resistance)
    ...
    rand_ctx_init_platform_entropy(c->options.min_platform_entropy, c->options.use_urandom);
#endif
}
...
#ifdef ENABLE_CRYPTO
  if (!o->replay)
    msg (M_FATAL, "ERROR: You have disabled Replay Protection (--no-replay) which may make " PACKAGE_NAME " less secure");
  if (!o->use_iv)
    msg (M_FATAL, "ERROR: You have disabled Crypto IVs (--no-iv) which may make " PACKAGE_NAME " less secure");
