		 ...
		 const struct frame* frame)
{
  ...
  if (buf->len > 0 && opt->key_ctx_bi)
      ...
      if (ctx->cipher)
	{
	  ...
	  /* Fox-IT Hardening: do not support CFB/OFB */
	  else /* We only support CBC mode right now */
	    {
	      ASSERT (0);
	    }
	       ...
	       bool cfb_ofb_allowed, bool warn)
{
  ...
  if (ciphername && ciphername_defined)
    {
      ...
      if (!is_allowed_data_channel_cipher(translate_cipher_name_from_openvpn(ciphername)))
    ...
    {
      /* Fox-IT hardening: force crypto. */
      msg (M_FATAL, "******* ERROR *******: null cipher not supported");
    }
  if (authname && authname_defined)
    ...
    {
      /* Fox-IT hardening: force crypto. */
      msg (M_FATAL, "******* ERROR *******: null HMAC not supported");
    }
}
