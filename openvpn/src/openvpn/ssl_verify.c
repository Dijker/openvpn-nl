...
verify_cert_set_env(struct env_set *es, openvpn_x509_cert_t *peer_cert, int cert_depth,
    const char *subject, const char *common_name, const char *issuer
...
verify_cert(struct tls_session *session, openvpn_x509_cert_t *cert, int cert_depth)
{
  ...
  /* get the X509 issuer */
  issuer = x509_get_issuer(cert, &gc);
  if (!issuer)
    {
        msg (D_TLS_ERRORS, "VERIFY ERROR: depth=%d, could not extract X509 "
  ...
  if (!string_mod_remap_name (common_name, COMMON_NAME_CHAR_CLASS))
    {
	msg (D_TLS_ERRORS, "VERIFY ERROR: depth=%d, illegal characters in "
	    "common name string from certificate", cert_depth);
	goto cleanup;
    }
  ...
  verify_cert_set_env(opt->es, cert, cert_depth, subject, common_name, issuer
#ifdef ENABLE_X509_TRACK
      , opt->x509_track
#endif
      );
