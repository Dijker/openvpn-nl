...
int entropy_gather_blocking(entropy_context *ec)
{
  int i;

  /* Try 10 gather operations */
  for (i = 0; i < 10; ++i)
  ...
  if (entropy_threshold_check(ec) != 0)
    {
       msg(M_INFO, "INFO: Gathering entropy, this may take some time");
       do {
         sleep(1);
         if (entropy_gather(ec) != 0)
           return 0;
       } while (entropy_threshold_check(ec) != 0);
    }
  return 1;
}
