...
static const char usage_message[] =
  ...
  "--min-platform-entropy: The minimum number of bytes of platform entropy\n"
  "                        required for a reseed of the random number generator.\n"
#ifndef WIN32
  "--use-urandom   : Use /dev/urandom instead of /dev/random as the platform "
  "                  entropy source. WARNING, this has security implications!\n"
#endif
#endif
#ifdef ENABLE_SSL
  "\n"
...
#endif
  "\n"
  "Generate a random key (for use in TA keys encryption mode):\n"
...
#endif
#ifdef ENABLE_CRYPTO_POLARSSL
  o->min_platform_entropy = FOX_ENTROPY_MIN_PLATFORM;
  o->use_urandom = false;
#endif
#ifdef ENABLE_SSL
  o->key_method = 2;
...
#ifdef ENABLE_SSL
  if (options->tls_server + options->tls_client > 1)
    msg (M_USAGE, "specify only one of --tls-server or --tls-client");
...
#endif
#ifdef ENABLE_CRYPTO_POLARSSL
        buf_printf (&out, ",min-platform-entropy %d", o->min_platform_entropy);
#endif
      }
...
/*      VERIFY_PERMISSION (OPT_P_MTU); */
  ...
  else if (streq (p[0], "min-platform-entropy"))
    {
       ...
       VERIFY_PERMISSION (OPT_P_GENERAL);
       min_platform_entropy = atoi(p[1]);

       if (min_platform_entropy < 0)
         {
           msg (msglevel, "Bad min-platform-entropy: %s", p[1]);
           goto err;
         }
  ...
  else if (streq (p[0], "use-urandom"))
    {
      ...
      /* CLOCK_MONOTONIC_RAW is not adjusted by adjtime and NTP on Linux */
      if ( clock_gettime(CLOCK_MONOTONIC_RAW, &tp) != -1 ) {
#else
      if ( clock_gettime(CLOCK_MONOTONIC, &tp) != -1 ) {
#endif
	if ( tp.tv_sec < MIN_URANDOM_UPTIME) {
	  msg (msglevel, "Error: using /dev/urandom is unsafe when system uptimeis lower than %d seconds.", MIN_URANDOM_UPTIME);
	  goto err;
	}
      } else {
	msg (msglevel|M_ERRNO, "Error retrieving system uptime");
	goto err;
      }

      options->use_urandom = true;
    }
#endif
#endif
#ifdef ENABLE_SSL
