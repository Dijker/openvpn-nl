...
char *
x509_get_issuer(x509_cert *cert, struct gc_arena *gc)
{
  ...
  ret = x509parse_dn_gets( tmp_issuer, MAX_SUBJECT_LENGTH-1, &cert->issuer );
  if (ret > 0)
    {
      /* Reject issuer with null characters, should be C string compatible */
      if (ret == strlen(tmp_issuer))
	{
...
x509_verify_crl(const char *crl_file, x509_cert *cert, const char *subject)
{
  ...
  if( x509parse_time_expired( &crl.next_update ) )
    {
      msg (M_WARN, "CRL CHECK: %s is EXPIRED", crl_file);
    }
