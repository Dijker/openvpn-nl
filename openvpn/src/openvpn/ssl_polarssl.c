...
tls_ctx_restrict_ciphers(struct tls_root_ctx *ctx, const char **ciphers)
{
  ...
  ASSERT (NULL != ctx);
  ASSERT (NULL != ciphers[0]);

  /* Get number of ciphers */
  for (i = 0, cipher_count = 1; NULL != ciphers[i]; i++) {
    cipher_count++;
  }
  ...
  for (i = 0, cipher_count = 1; NULL != ciphers[i]; i++) {
    ctx->allowed_ciphers[i] = ssl_get_ciphersuite_id( ciphers[i] );
  }
  ...
  if (!strcmp (cert_file, INLINE_FILE_TAG) && cert_file_inline)
    {
	  ...
	  strlen(cert_file_inline)))
        msg (M_FATAL, "Cannot load inline certificate file, please verify format and key size (1024-4096 bits)");
    }
  else
    {
      if (0 != x509parse_crtfile(ctx->crt_chain, cert_file))
	msg (M_FATAL, "Cannot load certificate file %s, please verify format and key size (1024-4096 bits).", cert_file);
    }
}
    ...
    const struct tls_root_ctx *ssl_ctx, bool is_server, struct tls_session *session)
{
  ...
  if (0 == ssl_init(ks_ssl->ctx))
	...
	{
	  int ret = 0;
	  ret = ssl_set_dh_param_ctx (ks_ssl->ctx, ssl_ctx->dhm_ctx );

	  if (POLARSSL_ERR_SSL_BAD_DHM_SIZE == ret)
	    dmsg (M_FATAL, "PolarSSL could not load DHM parameters: size should be 2048");
	  else if (ret)
	    dmsg (M_FATAL, "PolarSSL had trouble loading DHM parameters");
	}
...
show_available_tls_ciphers (const char *cipher_list)
{
  ...
  while (*ciphers != 0)
    {
      const char *cipher_name = ssl_get_ciphersuite_name(*ciphers);

      if (is_allowed_tls_cipher(cipher_name))
	printf ("%s\n", cipher_name);
