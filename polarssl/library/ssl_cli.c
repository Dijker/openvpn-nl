...
static int ssl_parse_server_key_exchange( ssl_context *ssl )
{
    ...
    if(
        ...
        ssl->session_negotiate->ciphersuite != TLS_DHE_RSA_WITH_DES_CBC_SHA &&
    ...
    if( ssl->handshake->dhm_ctx.len < POLARSSL_DHM_MIN_SIZE/8 || ssl->handshake->dhm_ctx.len > POLARSSL_DHM_MAX_SIZE/8 )
    {
        SSL_DEBUG_MSG( 1, ( "bad server key exchange message (invalid DHM P length)" ) );
        return( POLARSSL_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
    }
...
static int ssl_write_certificate_verify( ssl_context *ssl )
{
    ...
    else
    {
        ...
        if(
            ...
            ssl->session_negotiate->ciphersuite == TLS_RSA_WITH_AES_256_GCM_SHA384 ||
#endif
            ssl->session_negotiate->ciphersuite == TLS_DHE_RSA_WITH_AES_256_GCM_SHA384 )
        {
            hash_id = SIG_RSA_SHA384;
