                 ...
                 unsigned int nbits, int exponent )
{
    ...
    if( f_rng == NULL || nbits < POLARSSL_RSA_MIN_KEY_SIZE || nbits > POLARSSL_RSA_MAX_KEY_SIZE || exponent < 3 )
...
int rsa_check_pubkey( const rsa_context *ctx )
{
    ...
    if( mpi_msb( &ctx->N ) < POLARSSL_RSA_MIN_KEY_SIZE ||
        mpi_msb( &ctx->N ) > POLARSSL_RSA_MAX_KEY_SIZE  )
        return( POLARSSL_ERR_RSA_KEY_CHECK_FAILED );
